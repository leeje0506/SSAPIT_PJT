...
public class BoardRestController {
	...
	@GetMapping("/board")
	@ApiOperation(value="게시글 전체 조회", notes="검색조건도 넣으면 같이 가져온다.")
...
//		List<Board> list = boardService.getList(); //전체 조회
	...
	//5. 수정
	/*
	 * @PutMapping("/board") //JSON 형태의 데이터로 넘어왔을 떄 처리하고 싶은데? public
	 * ResponseEntity<Void> update(@RequestBody Board board){
	 * boardService.modifyBoard(board); //위와같은 상황 대비
	 * 
	 * return new ResponseEntity<Void>(HttpStatus.OK); }
	 */
